...
for seed in [20260602, 20260603, 20260604]:
    RUNS.append({
        ...
        "dir": Path(f"results/chest_baselines/C2_efficientnet_b0_compiler_compatible_binary_pool_seed{seed}"),
    })
...
for seed in [20260602, 20260603, 20260604]:
    RUNS.append({
        ...
        "dir": Path(f"results/chest_baselines/C3_efficientnet_b0_naive_binary_pool_all_abnormal_seed{seed}"),
    })

OUT_SEED = Path("results/tables/chest_efficientnet_b0_c2_c3_seed_results.csv")
OUT_AGG = Path("results/tables/chest_efficientnet_b0_c2_c3_aggregate_results.csv")
OUT_REPORT = Path("results/tables/chest_efficientnet_b0_c2_c3_report.md")
OUT_FIG = Path("results/figures/fig_chest_efficientnet_b0_c2_c3_macro_f1.png")
...
def main():
    ...
    plt.title("Chest X-ray EfficientNet-B0: compatible vs naive binary pooling")
    for i, row in fig_df.reset_index(drop=True).iterrows():
        plt.text(i, row["macro_f1_mean"] + 0.025, f"{row['macro_f1_mean']:.3f}", ha="center")
    plt.tight_layout()
    plt.savefig(OUT_FIG, dpi=250)
    plt.close()

    lines = []
    lines.append("# Chest X-ray EfficientNet-B0 C2 vs C3 Result Report\n")
    ...
    print("===== CHEST EFFICIENTNET-B0 C2/C3 AGGREGATION COMPLETE =====")
    ...
    print(f"Chest EfficientNet-B0 C2 - C3 macro F1 difference: {diff:.4f}")
